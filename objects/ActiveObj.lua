local ActiveObj = Class.create("ActiveObj", Entity, {health = 1})

----------------------Initialization-----------------
function ActiveObj:init( x, y )
	self.x = x
	self.y = y
end

function ActiveObj:create()
	--set default stats
	self.maxJumpTime = 9
...
function ActiveObj:setSpeedModifier(modifier)
	local velX, velY = self.body:getLinearVelocity()
	self.body:setLinearVelocity(velX * modifier, velY * modifier)
	self.speedModifier = modifier
end

function ActiveObj:move( dt, body, forceX, forceY, isMovingX)
...
function ActiveObj:setFixture( shape, mass, isSensor)
...
function ActiveObj:onCollide(other, collision)
	if self.state == 1 and self.velY > 400 then
		self:landing()
	end
end

function ActiveObj:landing( )
...
-------------------------Physics--------------------------
function ActiveObj:testProximity(destinationX, destinationY, proximity)
...
function ActiveObj:setPosition(x,y)
	self.canTeleport = true
	self.newX = x
	self.newY = y
end

function ActiveObj:getDistanceToPoint( pointX, pointY )
	return math.sqrt(math.pow(pointX - self.x,2) + math.pow(pointY - self.y,2 ) )
end

function ActiveObj:setJumpThru( timer )
	-- lume.trace(timer)
...
function ActiveObj:processJumpThru()
...
function ActiveObj:mCheckJumpThru(fixture, x, y, xn, yn, fraction )
	if fixture then
...
function ActiveObj:getAngleToPoint(x, y)
	local angle = math.atan2(y - self.y, x - self.x)
	return angle
end

function ActiveObj:turnToPoint(x, y)
	if x > self.x then
		self.dir = 1
	else
		self.dir = -1
...
function ActiveObj:checkGround()
	local checkGroundY = self.y + (self.charHeight or self.height) + 4
	self.numContacts = 0
	self.numInt = 0
...
function ActiveObj:mCheckGround(fixture, x, y, xn, yn, fraction )
	if fixture then
		local other = fixture:getBody():getUserData()
		local category = fixture:getCategory()
		if other ~= nil then
