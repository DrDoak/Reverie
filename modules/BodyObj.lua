...
function BodyObj:setPassive(name,effect)
...
function BodyObj:processPassives( )
	for i,v in pairs(self.passiveEffects) do
		v( self ,self.passiveVars[i])
	end
end

function BodyObj:updateHealthShields()
...
function BodyObj:setSpecialState(stateObject, canMove,uninterruptable)
	--self.state = 4
	self.prepTime = -5
